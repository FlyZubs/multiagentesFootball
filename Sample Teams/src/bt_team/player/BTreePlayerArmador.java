...
public class BTreePlayerArmador extends Thread {
	...
	BTNode<BTreePlayerArmador> btree;
	
	
	public BTreePlayerArmador(PlayerCommander player, Vector2D home) {
		commander = player;
		homePosition = home;
	...
	private BTNode<BTreePlayerArmador> buildTree() {
		Selector<BTreePlayerArmador> raiz = new Selector<BTreePlayerArmador>("RAIZ");
		
		Sequence<BTreePlayerArmador> goleiroTree = new Sequence<BTreePlayerArmador>("Chuta a bola pra longe");
		goleiroTree.add(new DetectBallInArea());
		
		Sequence<BTreePlayerArmador> zagueiroTree = new Sequence<BTreePlayerArmador>("Defende e rouba a bola");
		zagueiroTree.add(new IfClosestPlayerToBall());
		zagueiroTree.add(new GoGetBall());
		
		Sequence<BTreePlayerArmador> attackTree = new Sequence<BTreePlayerArmador>("Avanca-para-Gol");
		attackTree.add(new IfClosestPlayerToBall());
		attackTree.add(new AdvanceWithBallToGoal());
		attackTree.add(new KickToScore());

		Sequence<BTreePlayerArmador> deffensiveTree = new Sequence<BTreePlayerArmador>("Rouba-Bola");
